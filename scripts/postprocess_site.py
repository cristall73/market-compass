...
def stabilize(root):
    ...
    if review or not state.get('candidates'):
        state={'reviewDate':today.isoformat(),'candidates':fresh}
        save(STATE,state)
        inv['selectionReview']={'reviewedToday':True,'reviewDate':today.isoformat(),'nextReviewAfterTradingDays':REVIEW_TRADING_DAYS}
    else:
        held=state['candidates']
        # Mantiene la composizione; i prezzi vengono comunque rinfrescati.
        refresh_prices(held)
        inv['candidates']=held
...
def italianize(root):
    ...
    for group in list(intel.get('market') or [])+list(intel.get('companies') or []):
        for n in group.get('news') or []:
            original=n.get('title') or ''
            n['originalTitle']=original
            n['title']=translate(original,cache)
            if n.get('whyItMatters'):n['whyItMatters']=translate(n['whyItMatters'],cache)
    save(CACHE,cache)

def impact(event):
    name=(event.get('title') or event.get('event') or '').lower()
    if any(x in name for x in ['cpi','inflation','ppi','inflazione']):
        return 'Se il dato supera le attese può aumentare la pressione sui tassi e pesare soprattutto su Nasdaq e titoli growth; se è inferiore alle attese tende a favorire azioni e obbligazioni.'
    if any(x in name for x in ['rate','interest','fed','ecb','bce','tassi']):
        return 'Toni o decisioni più restrittivi del previsto tendono a penalizzare azioni growth e sostenere la valuta; indicazioni più accomodanti possono produrre l’effetto opposto.'
    if any(x in name for x in ['payroll','employment','unemployment','lavoro']):
        return 'Un mercato del lavoro molto forte può mantenere alti i tassi; un rallentamento moderato può favorire i mercati, mentre un deterioramento brusco aumenta il rischio recessione.'
    if any(x in name for x in ['gdp','pil']):
        return 'Una crescita superiore alle attese sostiene i ciclici ma può mantenere pressione sui tassi; una crescita debole penalizza i ciclici e aumenta i timori di rallentamento.'
...
def economic_calendar(root):
    ...
    try:
        r=requests.get(url,params=params,headers={'Origin':'https://www.tradingview.com','User-Agent':'Mozilla/5.0'},timeout=20);r.raise_for_status();raw=r.json()
